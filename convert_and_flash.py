...
def create_tf_model_from_onnx(onnx_path: Path):
    ...
    # Inspect ONNX for input/output dims
    session = ort.InferenceSession(str(onnx_path))
    input_shape  = session.get_inputs()[0].shape  # [1, 156]
    output_shape = session.get_outputs()[0].shape # [1, 18]

    # Functional API for clarity
    ...
    outputs = tf.keras.layers.Dense(output_shape[1], name="logits")(x)

    model = tf.keras.Model(inputs, outputs)
    # Dummy call to build weights
    model.compile(optimizer="adam", loss="sparse_categorical_crossentropy")
    model.predict(np.zeros((1, input_shape[1]), dtype=np.float32))
